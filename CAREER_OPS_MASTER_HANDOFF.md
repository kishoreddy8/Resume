...
**DISCREPANCY FOUND (verified this session, must be resolved before/during Phase 3):** `.claude/skills/tailor-resume/SKILL.md` — a real, working, DOCX-generating resume-tailoring skill (see §25) — still instructs reading the Master Resume/Skills Inventory from the **legacy, single-candidate** `data/master/manifest.json`, not the current per-candidate `data/candidates/<candidateId>/master/`. Confirmed on disk: `data/master/` still exists, byte-identical to `data/candidates/1/master/` (both 15,417-byte resume.docx, 33,796-byte skills.docx) — coincidentally correct only because there is currently exactly one candidate. This skill has **not** been updated for multi-candidate awareness and would silently read the wrong candidate's files (or stale data) the moment a second candidate exists and this skill is invoked without modification. `CAREER_OPS_PHASE_2_5_CHECKPOINT.md` already documents `data/master/` as an intentional legacy fallback that "nothing reads from it anymore" — that claim is **false** for this skill specifically; it must be corrected or the skill must be updated before relying on it in a multi-candidate context.
...
**Important correction to a naive "nothing exists" assumption:** a real, working, DOCX-generating resume/cover-letter tailoring capability **already exists** — `.claude/skills/tailor-resume/` (invoked as `/tailor-resume`, a Claude Code skill, not an app feature). It is genuinely substantial, not a stub:
...
**What this means for Phase 3 planning:** the "generate a tailored resume" problem is largely already solved *as a manual, human-in-the-loop skill invocation*. What's genuinely missing is the **integration** between that skill and the rest of this app's data pipeline — and one **confirmed discrepancy** that must be resolved first (see §18): the skill currently reads the legacy single-candidate `data/master/` path, not the per-candidate path every other part of this app now uses.
...
- Multi-candidate awareness in the skill itself (the §18 discrepancy).
