...
function drivingPrompt(candidateId: number, resumeSha: string, skillsSha: string): string {
  return [
    ...
    "5. restrictedToEmployers ONLY when the Skills Inventory itself limits a skill to named",
    "   employers — wording like \"Kubernetes — Client A only\" or \"used solely at X\". List those",
    "   employers. Do NOT set it just because the resume happens to mention the skill under one",
    "   employer: a resume is compressed, and its silence is not a restriction. Omit the field",
    "   entirely when no explicit limit is stated.",
    "",
