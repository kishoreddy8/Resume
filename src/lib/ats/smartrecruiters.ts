...
export async function fetchSmartRecruitersJobs(
  ...
  const company = encodeURIComponent(companyIdentifier);
  ...
  const normalized = await Promise.all(selected.map((posting) => limit(async (): Promise<NormalizedJob> => {
    ...
    return {
      ...
      url: `https://jobs.smartrecruiters.com/${encodeURIComponent(companyIdentifier)}/${encodeURIComponent(posting.id)}`,
